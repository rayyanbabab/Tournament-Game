          ...
          {teams && teams.length > 0 ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {teams.map((team: any) => (
                <Link key={team.id} href={`/teams/${team.id}`}>
                  <Card className="border-border/50 hover:shadow-lg hover:border-primary/30 transition-all cursor-pointer h-full">
                    <CardHeader>
                      <div className="flex items-center gap-3">
                        <div className="h-12 w-12 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center overflow-hidden shrink-0">
                          {team.logo_url
                            ...
                            : <span className="text-lg font-extrabold text-primary">{team.name[0]?.toUpperCase()}</span>}
                        </div>
                        <div>
                          <CardTitle className="text-lg">{team.name}</CardTitle>
                          <CardDescription>{team.team_members?.[0]?.count || 0} anggota</CardDescription>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent>
            ...
            <Card className="border-border/50">
              <CardContent className="py-16 text-center">
                ...
                <p className="text-muted-foreground">Belum ada tim yang terdaftar. Jadilah yang pertama!</p>
              </CardContent>
            </Card>
          )}
        </div>
